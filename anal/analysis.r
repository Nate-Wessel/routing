...
ods %>%
	summarise( quantile(it_n,probs=.9) )

# what predicts entropy?
summary(lm( 
	entropy ~ grid_dist * from_grid + real_flow + crosses_sub + o_km_from_sub * d_km_from_sub, 
	data=ods
))
...
cairo_pdf('~/Dropbox/diss/the/figures/3/delta-ecdf.pdf',width=8,height=4)
	times %>% 
		...
		ggplot(data=.) + 
			...
			labs(y='Cumulative Probability',x='Travel Time Difference (minutes)') + 
			facet_wrap(~Strategy)
dev.off()
...
cairo_pdf('~/Dropbox/diss/the/figures/3/delta-T-KDE.pdf',width=6,height=3)
	ods2 %>% 
		rename( Habit=dm_hab90, Realtime=dm_real90 ) %>% 
		gather(Habit,Realtime,key='Strategy',value='d_90') %>%
		ggplot(data=.) + 
			geom_density(aes(d_90,fill=Strategy),alpha=.3,color=NA) + 
			coord_trans(limx=c(0,35)) + 
			labs(x='90th Percentile Travel Time Risk (Minutes)',y='Density') + 
			facet_wrap(~period) + 
			theme_minimal()
dev.off()
...
summary(lm( hab90 ~ entropy, data=ods2, weights=weight))
